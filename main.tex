%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% A Treatise on the Emergence of Reality from a Unified Holographic Principle %
% LaTeX Skeleton - Optimized for XeLaTeX & GitHub Actions                      %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[11pt, letterpaper]{report}

%----------------------------------------------------------------
% ESSENTIAL PACKAGES & COMPILER SPECIFIC
%----------------------------------------------------------------
...
\usepackage{xeCJK}      % For CJK character support

% Fix for headheight warning (added from your log suggestion)
...
% IMPORTANT FONT FIX:
% setCJKmainfont needs to be loaded correctly *before* CJK characters are used.
% Also, for text in author/title, ensure that the CJK characters are indeed handled by xeCJK.
% If they are not in the main font, xeCJK *should* pick them up, but sometimes explicit wrapping helps.
% However, the standard setup with xeCJK usually defaults to the specified CJK font for those characters.
% The error implies TeX Gyre Termes is trying to render them.
% This setup is generally correct, the issue is that XeLaTeX is defaulting to the main font for these characters.
% The primary fix is to ensure the document encoding is correct and XeLaTeX is used.
% This is the most standard setup for mixed scripts and should work if fonts are available.
...
\usepackage{xparse} % For custom command definitions for \lambdabar, \Ztwo etc.
...
\hypersetup{
    ...
    pdfauthor={Kokuno Yumeto, Miroku Akagi}, % This is where the CJK characters are
    bookmarksopen=true,
...
\newtheorem{conjecture}{Conjecture}[chapter] % ADDED: Definition for Conjecture environment
\newtheorem{postulate}{Postulate}[chapter]   % ADDED: Definition for Postulate environment
...
% (Adding definitions for \lambdabar, \Ztwo etc. to fix "Undefined control sequence" errors)
%----------------------------------------------------------------
% Spacetime
\newcommand{\BulkM}{M}
...
\DeclareMathOperator{\Trn}{\text{Tr}} % Ensure Tr is defined correctly for your usage, if needed
\newcommand{\lambdabar}{\lambda \kern-0.5em \raise0.5ex\hbox{\char'26}} % Correct definition for lambdabar for specific font. If TeX Gyre Termes doesn't have it, it might still give a warning.
\newcommand{\Ztwo}{\mathbb{Z}_2} % For Z2
\newcommand{\Zthree}{\mathbb{Z}_3} % For Z3
\newcommand{\Zfour}{\mathbb{Z}_4} % For Z4

%----------------------------------------------------------------
% HEADER AND FOOTER SETUP (looks solid)
%----------------------------------------------------------------
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\nouppercase{\leftmark}}
...
\begin{postulate}[Relativistic Landauer's Principle, Entropy Form]
...
\begin{lemma}[Verification as a Physical Process of Decoherence]
\label{lemma:verification_as_decoherence}
Quantum measurement is physically realized through the process of decoherence, as detailed in Section \ref{sec:explore_measurement}. A unitary interaction between the system (S) and an external environment/apparatus (E) creates an entangled state $|\Psi\rangle = \sum_i c_i |s_i\rangle|E_i\rangle$. The observer, being part of E, perceives the system through its reduced density matrix, which rapidly diagonalizes due to the orthogonality of the environmental states:
\begin{equation}
    \rho_S = \Tr_E(|\Psi\rangle\langle\Psi|) \xrightarrow{t > t_{deco}} \sum_i |c_i|^2 |s_i\rangle\langle s_i|.
\end{equation}
This process converts the quantum superposition into a classical probability distribution, yielding a definite outcome and thus "verifying" the state's properties with respect to the measurement basis $\{|s_i\rangle\}$.
\end{lemma}

\begin{lemma}[The Polynomial Timescale of Decoherence]
\label{lemma:decoherence_timescale}
The decoherence timescale, $t_{verify} \approx t_{deco}$, is determined by local system-environment coupling strengths and the properties of the environment's spectral density. It is fundamentally independent of the internal preparation complexity of the state $|\psi_{\text{prop}}\rangle$. For local interactions, the decoherence rate typically scales at most polynomially with the number of system qubits, $n$. Therefore, the verification time is polynomially bounded: $t_{verify} \sim \text{poly}(n)$.
\end{proof}

\subsubsection*{Conclusion of Proof}
We have proven from the physical principles of the framework that the minimal time resources required for the two computational tasks are fundamentally different:
\begin{itemize}
...
\bibitem{Chamseddine1997SpectralAction}
